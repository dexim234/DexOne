...
export function usePumpTokens({
  ...
  const loadTokens = useCallback(async () => {
    try {
      setIsLoading(tokens.length === 0); // показываем спиннер только при первой загрузке
      setError(null);
  ...
  }, [columnType, filters, tokens.length]);
  ...
  useEffect(() => {
    ...
    const intervalId = setInterval(loadTokens, refreshInterval);
