...
export default function ProfilePage() {
  ...
  const statCards = [
    ...
    },
    {
      ...
      trend: "+28.3%",
      transKey: "profile.totalProfit",
    },
    {
      label: "Net Profit",
      value: `$${tradeStats.netProfit.toLocaleString()}`,
      icon: TrendingUp,
      colorClass: "text-teal-500",
      trend: "+22.1%",
      transKey: "profile.netProfit",
    },
  ];
              ...
              <div className="flex items-center justify-between">
                <div>
                  <CardTitle className="flex items-center gap-2">
                    ...
                    {t("profile.tradingStats")}
                  </CardTitle>
                  <CardDescription>
                    {t("profile.tradingStatsDesc")}
                  </CardDescription>
                </div>
                <Button
              ...
              <CardContent>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  {statCards.map((stat, index) => (
                    <div
                      key={index}
                      className="p-6 rounded-2xl bg-muted/30 hover:bg-muted/50 transition-all"
                    >
                      <div className="flex items-center justify-between mb-3">
                        <div className="p-2.5 rounded-xl bg-background/50">
                          <stat.icon
                            className={`h-5 w-5 ${stat.colorClass}`}
                          />
                        </div>
                        <span className="text-sm font-semibold text-green-500 bg-green-500/10 px-2.5 py-1 rounded-lg">
                          {stat.trend}
                        </span>
                      </div>
          ...
          <div className="space-y-6 mt-4">
            ...
            <div className="p-4 rounded-xl bg-gradient-to-r from-teal-muted/50 to-transparent border border-teal/20">
              <div className="flex items-center gap-3 mb-3">
                ...
                <div>
                  <h4 className="font-bold text-lg">{t("referralModal.directReferrals")}</h4>
                  <p className="text-sm text-muted-foreground">
                    {t("referralModal.directDesc")}
                  </p>
                </div>
              </div>
            ...
            <div className="p-4 rounded-xl bg-gradient-to-r from-purple-muted/50 to-transparent border border-purple/20">
              <div className="flex items-center gap-3 mb-3">
                ...
                <div>
                  <h4 className="font-bold text-lg">{t("referralModal.indirectReferrals")}</h4>
                  <p className="text-sm text-muted-foreground">
                    {t("referralModal.indirectDesc")}
                  </p>
                </div>
              </div>
              <ul className="space-y-2 ml-13">
            ...
            <div className="p-4 rounded-xl bg-gradient-to-r from-pink-muted/50 to-transparent border border-pink/20">
              <div className="flex items-center gap-3 mb-3">
                ...
                <div>
                  <h4 className="font-bold text-lg">{t("referralModal.specialBonuses")}</h4>
                  <p className="text-sm text-muted-foreground">
                    {t("referralModal.bonusesDesc")}
                  </p>
                </div>
              </div>
            ...
            >
              {t("referralModal.startReferring")}
              <ArrowRight className="h-4 w-4" />
            </Button>
          </div>
        </DialogContent>
          ...
          <div className="space-y-6 mt-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="p-6 rounded-2xl bg-muted/30 text-center">
                <div className="text-4xl font-bold text-green-500">{tradeStats.winRate}%</div>
                <div className="text-sm text-muted-foreground mt-2">{t("profile.winRate")}</div>
              </div>
              <div className="p-6 rounded-2xl bg-muted/30 text-center">
                <div className="text-4xl font-bold text-blue-500">${tradeStats.totalProfit.toLocaleString()}</div>
                <div className="text-sm text-muted-foreground mt-2">{t("profile.totalProfit")}</div>
              </div>
              <div className="p-6 rounded-2xl bg-muted/30 text-center">
                <div className="text-4xl font-bold text-teal-500">${tradeStats.netProfit.toLocaleString()}</div>
                <div className="text-sm text-muted-foreground mt-2">{t("profileModal.netProfit")}</div>
              </div>
            </div>

            {/* Performance Chart Placeholder */}
            <div className="p-6 rounded-xl bg-muted/30">
              <h4 className="font-bold mb-4">{t("profileModal.performanceChart")}</h4>
